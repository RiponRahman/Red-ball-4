...
int background[2];
int background_x[] = {0,400},background_y[]={0,0};
...
{ for(int i=0;i<2;i++)
	{
		...
		if(background_x[i]+400<0) //left slide out of frame
...
void iDraw()
{
	...
	*/
	...
	//iShowImage(0, 0, 400, 400, image3);
		for(int i=0;i<2;i++)
		iShowImage(background_x[i],0,400,400,background[i]);
    ...
    iUnRotate();
	/*Time show
	Integer converted to string using sprintf
	*/
	iSetColor(255,0,0);
	sprintf(str_time,"%d\n",timecount);
...
int main()
{
	...
	iSetTimer(1000,change_background);
	dx = 5;
	dy = 7;
	...
	*/
	...
	background[0] = iLoadImage("images\\sky.jpg");
	background[1] = iLoadImage("images\\sky.jpg");
